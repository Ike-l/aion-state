        ...
        AS::Access: Accessor,
{
    ...
    pub fn is_owned(
        &self, 
        input: &ReceptionIsOwned<'_, CS::ResourceId>
    ) -> bool {
        trace_function!("Coordinated Reception Is Owned");

        self.reception.read().is_owned(input)
    }
    ...
    pub fn check_owner(
        &self,
        input: &ReceptionCheckOwner<'_, CS::Id, CS::ResourceId>
    ) -> ReceptionCheckOwnerResult {
        trace_function!("Coordinated Reception Check Owner");

        self.reception.read().check_owner(input)
    }
}
