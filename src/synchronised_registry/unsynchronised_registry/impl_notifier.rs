...
> Notifier<<<S as RegistryStorage>::Value as StoredValueTrait>::Value> for UnsynchronisedRegistry<S, RS, AS, OS, WS, BS, CS> 
    where 
        ...
        <S as RegistryStorage>::Value: StoredValueTrait,
{
    ...
    fn unregister_waiter(&self, input: &Self::AccessInput, waiter: &Arc<Mutex<crate::prelude::Waiter>>) {
    ...
    fn acquire_access<'a, AccessResult: AccessorResult<'a, <<S as RegistryStorage>::Value as StoredValueTrait>::Value>>(&'a self, input: Self::AccessInput) -> Result<AccessResult, Self::Error> {
        unsafe { Ok(self.acquire_access(RegistryAcquireAccess {
